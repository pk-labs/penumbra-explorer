    ...
    const mockActiveValidators = (count: number) =>
        Array.from({ length: count }).map(() => {
            const date = faker.date
                .recent({ days: faker.number.int({ max: 1000, min: 1 }) })
                .valueOf()
            ...
            return {
                ...
                uptime: faker.datatype.boolean({ probability: 0.7 })
                    ? 100
                    : faker.number.float({ max: 100, min: 95 }),
                votingPower: faker.number.int({ max: 999999, min: 100000 }),
                votingPowerPercentage: faker.number.float({ max: 10, min: 1 }),
            }
        })
    ...
    const mockInactiveValidators = (count: number) =>
        Array.from({ length: count }).map(() => {
            ...
            return {
                ...
                hash: faker.finance.bitcoinAddress(),
                initialTimeAgo: dayjs().to(date),
                stakedTokens: faker.number.int({ max: 500, min: 0 }),
                ...
                timestamp: date.valueOf(),
                uptime: faker.number.float({ max: 95, min: 0 }),
                votingPower: faker.number.int({ max: 500, min: 0 }),
                votingPowerPercentage: faker.number.float({ max: 1, min: 0.1 }),
            }
        })
